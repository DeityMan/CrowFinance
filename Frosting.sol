...
contract Frosting Token is ERC20, Ownable {
    ...
    uint256 public CRORewardsFee = 5;
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Crow: The router already has that address");
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        uint256 initialCROBalance = IERC20(CRO).balanceOf(address(this));

        swapTokensForCRO(tokens);
        uint256 newBalance = (IERC20(CRO).balanceOf(address(this))).sub(initialCROBalance);
        IERC20(CRO).transfer(_marketingWalletAddress, newBalance);
    }
    ...
    function swapAndSendDividends(uint256 tokens) private{
        swapTokensForCRO(tokens);
        uint256 dividends = IERC20(CRO).balanceOf(address(this));
        bool success = IERC20(CRO).transfer(address(dividendTracker), dividends);

        if (success) {
            dividendTracker.distributeCRODividends(dividends);
            emit SendDividends(tokens, dividends);
        }
    }
}

contract CrowDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    ...
    constructor() public DividendPayingToken("Crow_Dividend_Tracker", "Crow_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 100 * (10**18); //must hold 100+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "Crow_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "Crow_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Crow contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Crow_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Crow_Dividend_Tracker: Cannot update claimWait to same value");
